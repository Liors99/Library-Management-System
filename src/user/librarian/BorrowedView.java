...
public class BorrowedView extends JPanel {
	...
	public BorrowedView() {
		...
		gl_borrowedBacklog.setHorizontalGroup(gl_borrowedBacklog.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_borrowedBacklog.createSequentialGroup().addContainerGap().addContainerGap()));

		gl_borrowedBacklog.setVerticalGroup(gl_borrowedBacklog.createParallelGroup(Alignment.LEADING).addGroup(
				gl_borrowedBacklog.createSequentialGroup().addContainerGap().addContainerGap(56, Short.MAX_VALUE)));
		...
		/*
		 * Queries the database, and populate objs with the ResultSet given. We'll use
		 * this data to populate a jtable which has a list of all the items where the
		 * return date is not null (the item has been returned)
		 */
		try {
		...
		/*
		 * 1) Dialog box confirming if you want to charge the user a fee 2) No closes
		 * the box 3) Yes charges the user 4) When you click yes, you go to another
		 * function where I do my join and i grab the price of the book
		 */
		borrowedTable.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			@Override
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					...
					if (selectedRow >= 0) {
						...
						int value = JOptionPane.showConfirmDialog(null, "Charge user with damage fees?", userBorrowing,
						...
						/*
						 * If the user clicks yes, they agree to charging the user with a damage fee.
						 * This calls 2 queries -> One in which they update the user with a new balance
						 * -> Another where they delete the instance of the returned obj
						 */
						if (value == JOptionPane.YES_OPTION) {
							chargeUser(getFee(borrowedISBN), userBorrowing, returnedDate, borrowedISBN);
							JOptionPane.showMessageDialog(borrowedBacklog,
									"User charged with damages, book has been returned");
							tableModel.removeRow(selectedRow);
							db.updateCheckedOut(borrowedISBN);
							/*
							 * If the user clicks no, then it skips charging the user with a damage fee ->
							 * Deletes the instance of the returned object
							 */
	...
	/*
	 * Function will do the following: 1) Add fees onto the user 2) Delete the
	 * borrowedObj entry from the table, since return has been completed
	 */
